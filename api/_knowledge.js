// Kai's knowledge base for the serverless function.
// Kept identical in content to src/data/kaiKnowledge.js — see that file for the
// human-editable source. Duplicated here so /api has zero dependency on the Vite
// build pipeline (serverless functions are bundled independently by Vercel).
...
Basu's contact details — email, phone number, location, LinkedIn, GitHub, and portfolio URL — are already shown publicly elsewhere on this website (Contact section, Resume preview, footer). These are not private information, so if a visitor asks for Basu's phone number, email, or social links, Kai should share them directly and naturally, the way a helpful person would, instead of deflecting to "use the Contact form" or treating the request as off-limits. Kai should never invent a number or link that isn't listed in section 7 below.
...
Basu is a fresher with no prior full-time work experience, focused on building his career as an AI Agent Engineer. He is actively seeking internship and fresher-level roles in AI Agent development. He has built and deployed multiple production-style AI agent projects independently as portfolio work, including completing a real take-home technical assignment as part of a job application process. He has also delivered real freelance client work by directing AI coding agents end-to-end rather than hand-coding everything himself — see the Mehfil Cafe project in section 5.7.
...
Why hire Basu: He has already built and deployed 6+ real, working multi-agent AI systems — not tutorials, but production-style projects with real architecture decisions: a 7-agent LangGraph pipeline with phase-based routing and targeted refinement logic, a weighted scoring engine achieving 95% alignment with manual benchmarks, and a data-driven LLM evaluation platform with hallucination detection. He's also delivered a real freelance client project (Mehfil Cafe, a 3D web experience) by directing AI coding agents through the full build. He's self-taught his way into agentic AI from an EEE background.
...
Model Context Protocol (MCP): Yes — used MCP within Claude, connecting it to Airbnb and EXA Search connectors as part of hands-on agentic AI training, and used an MCP server for UI component intelligence on the Mehfil Cafe freelance project.
AI coding assistants & orchestration: Claude Code, Cowork, Claude Design (for AI-generated 3D/visual assets), MCP servers, and plugin marketplaces — used together to direct multi-turn AI-assisted development workflows, as demonstrated end-to-end on the Mehfil Cafe freelance project.
AI safety practices: Applies guardrails in agent designs to keep outputs safe, on-topic, and reliable.
Developer tools: Git, GitHub, VS Code, Cursor, Claude Code, Postman, Jupyter Notebook, Docker, Vercel, Python virtual environments (venv), pip.
...
5. PROJECTS — OVERVIEW
Basu has built 6+ AI agent projects: AI Resume Matching Agent, AI Career Mentor Agent, AI Tour Guide Agent, AIForge (LLM Evaluation), Blog Generation Agent, and Agentic ChatBot. Three are live and publicly deployed (Resume Matching Agent, Career Mentor Agent, Tour Guide Agent); all six have public source code on GitHub. He has also delivered a real freelance client project, Mehfil Cafe, described in section 5.7.
...
5.7 Freelance Work — Mehfil Cafe (AI-Directed 3D Web Experience)
Basu delivered a freelance project for a Delhi-based cafe client (Mehfil Cafe): a scroll-driven 3D website, built by directing AI coding agents end-to-end rather than hand-coding everything himself. He owned the full delivery — writing the product spec, designing a 14-step scroll-driven camera narrative and section layout, choosing the deployment stack, and orchestrating Claude Code and Cowork through multi-turn iteration to a shipped, SEO-optimized product.
He designed the 3D interior himself — furniture (a bistro chair, a cafe table, a triangular-cage pendant lamp) created in Claude Design and exported as GLB models — then directed Claude Code to integrate them into a photoreal Three.js scene with image-based lighting and procedurally-generated wood/plaster textures.
He also curated a project-specific AI toolchain (an MCP server for UI component intelligence, a design-token plugin, task-specific system prompts) and used it to run performance and QA cycles that cut mobile load time (LCP) from about 4 seconds to about 2 seconds on 4G, and fixed a mobile GPU-memory crash by tuning texture settings per device.
Why this matters: it shows Basu can direct AI agents to deliver a real, shipped client product end-to-end — not just agent backends, but AI-assisted frontend/3D delivery, performance tuning, and translating client requirements into a finished site.
Date: August 2026. Live at mehfilcafe.vercel.app. Tech: Claude Code, Cowork, Claude Design, MCP, Three.js, Vercel.
...
How to contact Basu: email basueps@gmail.com, phone +91-9311551167 (based in New Delhi, India), LinkedIn (linkedin.com/in/kumar-basu-singh), GitHub (github.com/mr-basu-singh), portfolio basusingh.vercel.app, or the Contact form on this site.
